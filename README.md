...
**[Try it →](https://paperlab.nawwara.studio/)**  ·  [the editor](https://paperlab.nawwara.studio/editor/) (desktop)  ·  [the reference](https://paperlab.nawwara.studio/docs/)  ·  [with your hands](https://paperlab.nawwara.studio/hands/) (webcam)  ·  [the FX Lab](https://paperlab.nawwara.studio/fx-lab/)  ·  [for coding agents](AGENTS.md)
...
Every default is a tune made in the [FX Lab](https://paperlab.nawwara.studio/fx-lab/), and every one of them is an option you can set. See [AGENTS.md](AGENTS.md) for the wiring.
...
| [`apps/editor`](apps/editor/) | the editor — Paper, Field and Stage, plus Hands (`/hands`) and the FX Lab (`/fx-lab`), each built from the same app in a pass of its own |
| [`apps/docs`](apps/docs/) | the reference site, with every behavior running live |
| [`tools/`](tools/) | browser harnesses — parity, perf, screenshots, the fire's checks, the README's motion |
| [`AGENTS.md`](AGENTS.md) · [`docs/llms.txt`](docs/llms.txt) | the agent-readable API reference |
| [`docs/design.md`](docs/design.md) | the design language the apps share, enforced by a test |
