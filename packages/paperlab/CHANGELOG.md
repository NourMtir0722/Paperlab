...
- 63f86bf: A piece that a `<Paper damage>` burn cuts loose from a cloth sheet now falls away cleanly.
  
  It always fell, because every spring across the cut breaks. But on a cut one particle wide, each burnt particle on the edge was averaged between the two sides. As the piece dropped, those particles were dragged down the gap after it, and both burnt edges were drawn as streaks. Now each one rides with the side it borders most. The triangles left across the opening gap are hidden once they stretch, so the piece leaves with its own burnt edge.
  
  A sheet that is still in one piece draws exactly as before.
  
  When a burn is played back from an earlier moment, the damage source hands back paper that had burnt away. The sheet is now laid out afresh instead of hauling fallen paper back up through itself.

## 0.7.0

### Minor Changes

- 5fac05e: **Paper can be damaged, and `paperlab/fx` is what damages it.**
  ...
  **What a burn looks like** was rebuilt against a visual spec, and it is what this release ships:
  ...
  - **`FxFireFluid`** — the flames are a small GPU fire simulation on a camera-facing plane: fuel released from a thin band along the hot rim burns only where air has reached it — premixed, mixed in from beside, or drawn in from in front and behind, which dense fuel keeps out — into heat, soot and smoke, so a tongue's fuel core is eaten from the outside and closes to a tip; buoyancy and curl-noise turbulence move it, and a pressure solve keeps it swirling instead of smearing. It is fed from the hot rim in uneven clusters (`flameAnchors`), never a uniform crown. Its render pass draws a flame's SHAPE from its soot — which forms from hot fuel and is gone where the fuel runs out or the gas cools — and its COLOUR from the heat that BURNING made (not the heat the gas left the paper with, which peaks at the root where a real flame is coolest), normalised, so a flame has a pale core, a saturated body and a torn tip rather than one saturated colour; a domain warp and a noise that bites only at the cool end give it detail finer than the grid, so tongues split instead of dissolving. What is drawn is advected with MacCormack's error correction rather than a single semi-Lagrangian step, which loses about half a cell of detail every step; `FxFireFluid`'s `sharp` prop turns it off, and it is off by default on the `low` tier, where its two extra passes a step are the first thing a throttled phone should give back. How the fire is DRAWN is separate from how it moves, and every term of it is a prop: colour follows intensity the way a hot body glows (burnt orange, orange, amber, yellow, and pale only at the hottest root), each tongue is smooth inside and defined by its outline, dense flame covers the paper instead of tinting it, and thin gas glows only as much as it covers. The flame is drawn in the four zones a real one has over a sheet, base to tip — a **root** (the blue chemiluminescent edge where fresh gas meets air, off by default because blue over cream reads lavender), a **core** (the hottest, densest soot, pale, and the only part that over-exposes), a **body** (luminous soot, yellow-orange, brighter than paper white) and a **tip** (soot cooling and burning off, dimmer and redder, where tongues tear). `zones` sets any part of them — each a colour and a brightness in multiples of paper white, with the boundaries between them, the outline's softness and how much the tip tears — laid over `FIRE_ZONES`; `fireZones()` fills a partial one in, and `FireZones` / `FireZonesInput` type them. Beside the zones: `heatScale`, `sootScale`, `contrast`, `opacity`, `thin`, `warm` (how long a reset fire runs unseen, so a seeked-to frame does not show the starting vortex of a plume begun from still air) and `sharp` (MacCormack advection). Their defaults are exported from `paperlab/fx` as `FIRE_*` constants. `FireFluid` is the solver on its own; `fireFluidDefaults` and `fireFluidControls` are its parameters (emission, fuel, premixed and ambient oxygen, heat, smoke, radial impulse, velocity, burn rate, gas expansion, buoyancy, cooling, persistence, air mixing, turbulence and its scale, vorticity, wind, and how long a spot of the rim goes on smoking once its flame is out — `smokeAfter`), and `fxQualityTiers` gains `fluid` for its grid sizes — velocity at half the drawn grid's resolution, stepped 120 times a second. Its noise is hashed without `sin`, whose precision differs from GPU to GPU, so a fire looks the same on every device. Where half-float targets are missing it draws its `fallback` instead.
